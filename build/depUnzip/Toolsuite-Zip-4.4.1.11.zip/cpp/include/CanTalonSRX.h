...
#define CTR_SigNotUpdated_MESSAGE "CTRE CAN Signal Not Updated"
  ...
  CTR_Code GetMotionProfileStatus(uint32_t &flags, uint32_t &profileSlotSelect,
	...
	CTR_Code SetThrottleBump(int32_t throttleBump);
	CTR_Code GetMotionMagic_ActiveTrajVelocity(int32_t & param);
	CTR_Code GetMotionMagic_ActiveTrajPosition(int32_t & param);
	CTR_Code SetCurrentLimEnable(bool bEnableCurrLim);
	CTR_Code SetPersistentStorageWriteTimeout(
			uint32_t persStorageWriteTimeoutMs);
 ...
 */
  ...
  CTR_Code GetClearPosOnIdx(int &param);
  CTR_Code GetClearPosOnLimR(int &param);
  CTR_Code GetClearPosOnLimF(int &param);
